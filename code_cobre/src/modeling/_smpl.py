...
class SMPL(nn.Module):
    ...
    def __init__(self, gender='neutral'):
        ...
        self.register_buffer('posedirs', torch.FloatTensor(smpl_model['posedirs'])) #[6890, 3, 20]
        ...
        self.register_buffer('parent', torch.LongTensor([id_to_col[self.kintree_table[0, it].item()] for it in range(1, self.kintree_table.shape[1])])) #[23]
    ...
    def forward(self, pose, beta):
        device = pose.device
        batch_size = pose.shape[0] #[20, 72]
        v_template = self.v_template[None, :] #[1, 6890, 3]
        shapedirs = self.shapedirs.view(-1,10)[None, :].expand(batch_size, -1, -1) #[20, 20670, 10]
        beta = beta[:, :, None] #[20, 10, 1]
        v_shaped = torch.matmul(shapedirs, beta).view(-1, 6890, 3) + v_template #[20, 6890, 3]
        # batched sparse matmul not supported in pytorch
        J = []
        for i in range(batch_size):
            J.append(torch.matmul(self.J_regressor, v_shaped[i]))
        J = torch.stack(J, dim=0) #[20, 24, 3]
        ...
        # input it rotmat: (bs,72)
            ...
            R = rodrigues(pose_cube).view(batch_size, 24, 3, 3) #[20, 24, 3, 3]
            R = R.view(batch_size, 24, 3, 3) #[20, 24, 3, 3]
        I_cube = torch.eye(3)[None, None, :].to(device) #[1, 1, 3, 3]
        # I_cube = torch.eye(3)[None, None, :].expand(theta.shape[0], R.shape[1]-1, -1, -1)
        lrotmin = (R[:,1:,:] - I_cube).view(batch_size, -1) #[20, 207]
        posedirs = self.posedirs.view(-1,207)[None, :].expand(batch_size, -1, -1) #[20, 20670, 207]
        v_posed = v_shaped + torch.matmul(posedirs, lrotmin[:, :, None]).view(-1, 6890, 3) #[20, 6890, 3]
        J_ = J.clone() #[20, 24, 3]
        J_[:, 1:, :] = J[:, 1:, :] - J[:, self.parent, :]
        G_ = torch.cat([R, J_[:, :, :, None]], dim=-1) #[20, 24, 3, 4]
        pad_row = torch.FloatTensor([0,0,0,1]).to(device).view(1,1,1,4).expand(batch_size, 24, -1, -1) #[20, 24, 1, 4]
        G_ = torch.cat([G_, pad_row], dim=2) #[20, 24, 4, 4]
        G = [G_[:, 0].clone()] #[20, 4, 4]
        for i in range(1, 24):
            G.append(torch.matmul(G[self.parent[i-1]], G_[:, i, :, :]))
        G = torch.stack(G, dim=1) #[20, 24, 4, 4]

        rest = torch.cat([J, torch.zeros(batch_size, 24, 1).to(device)], dim=2).view(batch_size, 24, 4, 1) #[20, 24, 4, 1]
        zeros = torch.zeros(batch_size, 24, 4, 3).to(device) #[20, 24, 4, 3]
        rest = torch.cat([zeros, rest], dim=-1) #[20, 24, 4, 4]
        rest = torch.matmul(G, rest) #[20, 24, 4, 4]
        G = G - rest
        T = torch.matmul(self.weights, G.permute(1,0,2,3).contiguous().view(24,-1)).view(6890, batch_size, 4, 4).transpose(0,1) #[20, 6890, 4, 4]
        rest_shape_h = torch.cat([v_posed, torch.ones_like(v_posed)[:, :, [0]]], dim=-1) #[20, 6890, 4]
        v = torch.matmul(T, rest_shape_h[:, :, :, None])[:, :, :3, 0] #[20, 6890, 3]
        return v
    ...
    def get_h36m_joints(self, vertices):
        ...
        """ #J_regressor[17, 6890]
        joints = torch.einsum('bik,ji->bjk', [vertices, self.J_regressor_h36m_correct])#vertices[20, 6890, 3]
        return joints #joints [20, 17, 3]
...
class Mesh(object):
    ...
    def __init__(self, filename=cfg.SMPL_sampling_matrix,
        ...
        # self._A = [a.to(device) for a in self._A] #[6890, 6890][1723, 1723][431, 431]
        self._U = [u.to(device) for u in self._U] #[6890, 1723][1723, 431]
        self._D = [d.to(device) for d in self._D] #[1723, 6890][431, 1723]

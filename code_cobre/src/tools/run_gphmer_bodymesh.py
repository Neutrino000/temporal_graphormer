...
def mean_per_joint_position_error(pred, gt, has_3d_joints): #[20, 14, 3][20, 14, 4][20]
    """ 
    Compute mPJPE
    """
...
def keypoint_3d_loss(criterion_keypoints, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d, device):
    ...
    """
    conf = gt_keypoints_3d[:, :, -1].unsqueeze(-1).clone()#[20, 14, 1]

    gt_keypoints_3d = gt_keypoints_3d[:, :, :-1].clone() #[20, 14, 3]
    gt_keypoints_3d = gt_keypoints_3d[has_pose_3d == 1] #[20, 14, 3]
    conf = conf[has_pose_3d == 1] #[20, 14, 1]
    pred_keypoints_3d = pred_keypoints_3d[has_pose_3d == 1] #[20, 14, 3]
    if len(gt_keypoints_3d) > 0:
        gt_pelvis = (gt_keypoints_3d[:, 2,:] + gt_keypoints_3d[:, 3,:]) / 2 #[20, 14, 1]
        gt_keypoints_3d = gt_keypoints_3d - gt_pelvis[:, None, :]#[20, 14, 3]
        pred_pelvis = (pred_keypoints_3d[:, 2,:] + pred_keypoints_3d[:, 3,:]) / 2#[20, 14, 1]
        pred_keypoints_3d = pred_keypoints_3d - pred_pelvis[:, None, :]#[20, 14, 3]
...
def run(args, train_dataloader, val_dataloader, Graphormer_model, smpl, mesh_sampler, renderer):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        images = images.cuda(args.device) #[20, 3, 224, 224]
        gt_2d_joints = annotations['joints_2d'].cuda(args.device) #[20, 24, 3]
        gt_2d_joints = gt_2d_joints[:,cfg.J24_TO_J14,:]#[20, 14, 3]
        has_2d_joints = annotations['has_2d_joints'].cuda(args.device)#[20]

        gt_3d_joints = annotations['joints_3d'].cuda(args.device)#[20, 24, 4]
        gt_3d_pelvis = gt_3d_joints[:,cfg.J24_NAME.index('Pelvis'),:3]#[20, 1, 3]
        gt_3d_joints = gt_3d_joints[:,cfg.J24_TO_J14,:] #[20, 14, 4]
        gt_3d_joints[:,:,:3] = gt_3d_joints[:,:,:3] - gt_3d_pelvis[:, None, :]
        has_3d_joints = annotations['has_3d_joints'].cuda(args.device) #[20]

        gt_pose = annotations['pose'].cuda(args.device) #[20, 72]
        gt_betas = annotations['betas'].cuda(args.device) #[20, 10]
        has_smpl = annotations['has_smpl'].cuda(args.device) #[20]
        mjm_mask = annotations['mjm_mask'].cuda(args.device) #[20, 14, 1]
        mvm_mask = annotations['mvm_mask'].cuda(args.device) #[20, 431, 1]

        # generate simplified mesh
        gt_vertices = smpl(gt_pose, gt_betas) #[20, 6890, 3]
        gt_vertices_sub2 = mesh_sampler.downsample(gt_vertices, n1=0, n2=2)#[20, 431, 3]
        gt_vertices_sub = mesh_sampler.downsample(gt_vertices)#[20, 1723, 3]

        # normalize gt based on smpl's pelvis 
        gt_smpl_3d_joints = smpl.get_h36m_joints(gt_vertices)#[20, 17, 3]
        gt_smpl_3d_pelvis = gt_smpl_3d_joints[:,cfg.H36M_J17_NAME.index('Pelvis'),:]#[20, 1, 3]
        gt_vertices_sub2 = gt_vertices_sub2 - gt_smpl_3d_pelvis[:, None, :]#[20, 431, 3]
            
        # prepare masks for mask vertex/joint modeling
        mjm_mask_ = mjm_mask.expand(-1,-1,2051) #[20, 14, 2051]
        mvm_mask_ = mvm_mask.expand(-1,-1,2051) #[20, 431, 2051]
        meta_masks = torch.cat([mjm_mask_, mvm_mask_], dim=1)#[20, 445, 2051]
        ...
        #[20, 3][20, 14, 3][20, 431, 3][20, 1723, 3][20, 6890, 3]
        # normalize gt based on smpl's pelvis 
        gt_vertices_sub = gt_vertices_sub - gt_smpl_3d_pelvis[:, None, :]#[20, 1723, 3]
        gt_vertices = gt_vertices - gt_smpl_3d_pelvis[:, None, :]#[20, 6890, 3]

        # obtain 3d joints, which are regressed from the full mesh
        pred_3d_joints_from_smpl = smpl.get_h36m_joints(pred_vertices)#[20, 17, 3]
        pred_3d_joints_from_smpl = pred_3d_joints_from_smpl[:,cfg.H36M_J17_TO_J14,:]#[20, 14, 3]

        # obtain 2d joints, which are projected from 3d joints of smpl mesh
        pred_2d_joints_from_smpl = orthographic_projection(pred_3d_joints_from_smpl, pred_camera) #[20, 14, 3]

        pred_2d_joints = orthographic_projection(pred_3d_joints, pred_camera)#[20, 14, 2]
        ...
        # compute 3d vertex loss 0.33*loss1+0.33*loss2+0.33*loss3
        ...
        # compute 2d joint loss [20, 14, 3][20, 14, 4]
...
def run_validate(args, val_loader, Graphormer_model, criterion, criterion_vertices, epoch, smpl, mesh_sampler):
    ...
    with torch.no_grad():
        ...
        for i, (img_keys, images, annotations) in enumerate(val_loader):
            ...
            images = images.cuda(args.device)  #[20, 3, 224, 224]
            gt_3d_joints = annotations['joints_3d'].cuda(args.device) #[20, 24, 4]
            gt_3d_pelvis = gt_3d_joints[:,cfg.J24_NAME.index('Pelvis'),:3] #[20, 3]
            gt_3d_joints = gt_3d_joints[:,cfg.J24_TO_J14,:]  #[20, 14, 4]
            gt_3d_joints[:,:,:3] = gt_3d_joints[:,:,:3] - gt_3d_pelvis[:, None, :]
            ...
            gt_vertices = smpl(gt_pose, gt_betas) #[20, 6890, 3]
            gt_vertices_sub = mesh_sampler.downsample(gt_vertices) #[20, 1723, 3]
            gt_vertices_sub2 = mesh_sampler.downsample(gt_vertices_sub, n1=1, n2=2) #[20, 431, 3]
            ...
            #[20, 3][20, 14, 3][20, 431, 3][20, 431, 1723][20, 6890, 3]
            # obtain 3d joints from full mesh
            pred_3d_joints_from_smpl = smpl.get_h36m_joints(pred_vertices) #[20, 17, 3]

            pred_3d_pelvis = pred_3d_joints_from_smpl[:,cfg.H36M_J17_NAME.index('Pelvis'),:]#[20, 1, 3]
            pred_3d_joints_from_smpl = pred_3d_joints_from_smpl[:,cfg.H36M_J17_TO_J14,:] #[20, 14, 3]
            pred_3d_joints_from_smpl = pred_3d_joints_from_smpl - pred_3d_pelvis[:, None, :] #[20, 14, 3]
            pred_vertices = pred_vertices - pred_3d_pelvis[:, None, :] #[20, 17, 3]

            # measure errors #pred_vertices[20, 17, 3]gt_vertices[20, 6890, 3] has_smpl[20]
            ...
            #pred_3d_joints_from_smpl[20, 14, 3]gt_3d_joints[20, 14, 4]has_3d_joints[20]
...
def parse_args():
    ...
    parser.add_argument("--train_yaml", default='/media/son/eden/Projects/MeshGraphormer-main/datasets/3dpw/train.yaml', type=str, required=False,
                        help="Yaml file with all data for training.")
    parser.add_argument("--val_yaml", default='/media/son/eden/Projects/MeshGraphormer-main/datasets/3dpw/test_has_gender.yaml', type=str, required=False,
    ...
    #########################################################
    parser.add_argument("--model_name_or_path", default='/media/son/eden/Projects/MeshGraphormer-main/src/modeling/bert/bert-base-uncased/', type=str, required=False,
                        help="Path to pre-trained transformer model or model type.")
    parser.add_argument("--resume_checkpoint", default='/media/son/eden/Projects/MeshGraphormer-main/models/graphormer_release/graphormer_h36m_state_dict.bin', type=str, required=False,
...
def main(args):
    ...
    hidden_feat_dim = [int(item) for item in args.hidden_feat_dim.split(',')] #[1024, 256, 64]
    ...
    if args.run_eval_only==True and args.resume_checkpoint!=None and args.resume_checkpoint!='None' and 'state_dict' not in args.resume_checkpoint:
        ...
        for i in range(len(output_feat_dim)): #[512, 128, 3]
            ...
            args.intermediate_size = int(args.hidden_size*args.interm_size_scale) #interm = 2
            ...
            config.mesh_type = args.mesh_type #body
        ...
        if args.arch=='hrnet':
            hrnet_yaml = '/media/son/eden/Projects/MeshGraphormer-main/models/hrnet/cls_hrnet_w40_sgd_lr5e-2_wd1e-4_bs32_x100.yaml'
            hrnet_checkpoint = '/media/son/eden/Projects/MeshGraphormer-main/models/hrnet/hrnetv2_w40_imagenet_pretrained.pth'
            hrnet_update_config(hrnet_config, hrnet_yaml)
            backbone = get_cls_net_gridfeat(hrnet_config, pretrained=hrnet_checkpoint)   #!!!!!!!!!!!!!!!!!
            logger.info('=> loading hrnet-v2-w40 model')
        elif args.arch=='hrnet-w64':
            hrnet_yaml = '/media/son/eden/Projects/MeshGraphormer-main/models/hrnet/cls_hrnet_w64_sgd_lr5e-2_wd1e-4_bs32_x100.yaml'
            hrnet_checkpoint = '/media/son/eden/Projects/MeshGraphormer-main/models/hrnet/hrnetv2_w64_imagenet_pretrained.pth'
            hrnet_update_config(hrnet_config, hrnet_yaml)
            backbone = get_cls_net_gridfeat(hrnet_config, pretrained=hrnet_checkpoint)    #!!!!!!!!!!!!!!!!!!!!!
